...
def linearccca_emb(train_audio_rgb_label, test_audio_rgb_label, output_size, beta):
    ...
    audioTrain -= 0#m0.reshape([1, -1]).repeat(data_num, axis=0)
    audioFeatTrain =0# np.dot(audioTrain, w0)
    rgbTrain -=0# m1.reshape([1, -1]).repeat(data_num, axis=0)
    rgbFeatTrain = 0#np.dot(rgbTrain, w1)
...
def ccca_emb(train_audio_rgb_label, test_audio_rgb_label, output_size, beta):
    audioTrain, rgbTrain, labelTrain = train_audio_rgb_label
    audioTest,  rgbTest,  labelTest  = test_audio_rgb_label
    trainDccaModel = True
    if (trainDccaModel):
...
def cross_fold_embed_read():
    ...
    for i in range(1):
        ...
        output_size = 10
        beta        = 0.2
        (audioFeatTrain, rgbFeatTrain, labelTrain), (audioFeatTest, rgbFeatTest, labelTest) = linearccca_emb(train_audio_rgb_label, test_audio_rgb_label, output_size,beta)
...
if __name__ == "__main__":
    ...
    for data_xy in cross_fold_embed_read():
        (audioFeatTrain, rgbFeatTrain), labelTrain, (audioFeatTest, rgbFeatTest), labelTest = data_xy
        viewx = audioFeatTest, rgbFeatTest
        c_matrix = utl.corr_compute(viewx, tag = "cosine")
        result_l = utl.metric(c_matrix, labelTest)
        print(result_l)
