...
def c_dcca_emb(loadfile, output_size, batch_size, Applied_CCA):
    ...
    data1 =  [audioTrain], [audioTrain], [list(audioTest)]
    data2 =  [rgbTrain],   [rgbTrain],   [list(rgbTest)]
    trainDccaModel = True
    model   = train_model(data1, data2, trainDccaModel, output_size, batch_size)
    print("Training finish! Start to test..........")
    featN   = test_model(data1, data2, trainDccaModel, output_size, batch_size)
    new_data = applid_linear_cca(featN, Applied_CCA)
    print("embedding...........",new_data[2][0].shape, new_data[2][1].shape)
    return (new_data[2][0], new_data[2][1]), labelTest

if __name__ == '__main__':
    Applied_CCA = True
    views, testLs = c_dcca_emb("Data/vegas_feature.h5", 10, 500, Applied_CCA)
    corr_matrix = corr_compute(views[0], views[1], tag = "cosine")
    result_list = metric(corr_matrix, testLs)
    print(result_list)
